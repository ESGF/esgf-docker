...
echo "[info] Running pre-init customisations from $ESGF_INIT_DIR"
if [ -d "$ESGF_INIT_DIR" ]; then
    for file in $(find $ESGF_INIT_DIR -mindepth 1 -type f -executable | sort -n); do
